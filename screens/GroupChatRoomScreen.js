...
const RoomScreen = ({ route }) => {
    ...
    //const currentName = currentUser.email.substring(0,currentUser.email.indexOf('@'));
    console.log("Sjdjndknsjkbnvjk========>>>>", currentUser)
    const thread = route.params.thread;

    const [messages, setMessages] = useState("");
    const [input, setInput] = useState("")
    const [data,setData] = useState("")
    console.log("dhvhjdsvgjckv===========>>>>>>>>>>>>>>",data)
    ...
    async function handleSend(messages, type) {
        const text = messages;
    ...
    const getUserDetail = () => {
        firestore().collection("Users").where('email', '==', currentUser.email).get().then(documentSnapshot => {
           setData(documentSnapshot.docs[0].data())         
        })}  
                                ...
                                return (
                                    currentUser.email == itemData.item.user.email ?
                                        ...
                                        <View style={styles.leftView}>
                                            <Text style={styles.msgText}>{itemData.item.user.name}</Text>
                                            <Text style={styles.msgText}>{itemData.item.text}</Text>
